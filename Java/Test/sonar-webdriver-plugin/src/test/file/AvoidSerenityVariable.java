...
public class TheInternetLoginPage extends Base {
    ...
    public void type() {
    	Serenity.setSessionVariable("correlationId").to(taskPage.lbl_order_package.getText().trim()); // Noncompliant {{Avoid using Serenity SessionVariable in tools project instead only use only in test project}}
    	
    	String myString=Serenity.sessionVariableCalled("file_type"); // Noncompliant {{Avoid using Serenity SessionVariable in tools project instead only use only in test project}}
    }
}
